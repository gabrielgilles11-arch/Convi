...
| Route | Mode | Why |
| :--- | :--- | :--- |
| `/`, `/about`, `/scenarios`, `/scenarios/[id]/words`, `/llms.txt` | Prerendered | Read by search and answer engines. Static HTML, no runtime cost, no cold start. |
| `/scenarios/[id]` | Prerendered **while `PAYWALL_ENABLED` is false** | `export const prerender = !PAYWALL_ENABLED`. Free content should be static; gated content cannot be. |
| `/practice` | Prerendered shell | The page is a shell. Its deck arrives from `/api/quiz-items`, which is where the entitlement check lives, so the deck never ships in the bundle. |
...
schema; they are the front door.

`/scenarios/[id]` is the one route whose mode is a consequence rather than a
choice, and getting it wrong is expensive in a way nothing warns you about. It
was rendered on demand so that premium categories could be gated per request —
correct while something was paid, and pure cost once nothing was. The sitemap
only lists pages that were built, so for as long as that flag stayed false the
sitemap held twelve URLs and not one of the forty scenario pages: the entire
search surface was invisible to anything that reads a sitemap. It follows the
flag now, and `getStaticPaths` enumerates every locale and category so it has
something to build.

## Being found

Three surfaces, and they are maintained differently on purpose.

- **Sitemap** — generated by `@astrojs/sitemap` from what was built, so it is
  right by construction as long as the pages are static. `/stats` is filtered
  out.
- **`/llms.txt`** — generated from the content files by `src/pages/llms.txt.ts`.
  ...
  Swedish and German editions after it was written was missing. Each row now
  carries the page's search title and its quick answer, so a model reading it
  gets a real phrase per row rather than a bare link.
- **`/about`** — the only hand-written page of the three, and the one that
  carries the full section index. That index is what puts all 44 scenario pages
  one click from every page on the site; before it they were reachable only
  through their own edition's index.

No `hreflang`. The three editions are not translations of one another — every
page on the site is written in English, about a different language — so
declaring them as alternates would be a claim that is simply false.
